...
#### Опис
`MainWindow` представляє головне вікно програми, яке відображає інформацію про персонажа, список квестів та дозволяє взаємодіяти з ними.

#### Публічні методи

- `MainWindow(QWidget *parent = nullptr)`: Конструктор класу, ініціалізує головне вікно програми.
  
- `void addActiveQuest(Quest* quest)`: Додає активний квест до персонажа.

- `void updateInfoOnCharacter()`: Оновлює інформацію про характеристики персонажа у вікні.

- `void updateInfoOnQuest()`: Оновлює список активних квестів у вікні.

- `void handleQuestInfoClosed()`: Обробляє закриття вікна з інформацією про квест.

- `void deleteQuest()`: Видаляє вибраний квест у головному вікні.

- `void questComplete()`: Викликається при завершенні квесту, оновлює характеристики персонажа та можливо додає новий предмет до інвентаря.

- `void updateCharacterEquipment(const std::vector<Item>& equipment)`: Оновлює екіпіровку персонажа у вікні залежно від інвентаря.

#### Приватні члени

- `Ui::MainWindow *ui`: Вказівник на об'єкт, що містить візуальні компоненти головного вікна.
  
- `Character character`: Об'єкт, що представляє персонажа гри.

- `Quest quest`: Об'єкт, що представляє квест.

#### Публічні слоти

- `on_change_space_clicked()`: Обробник події натискання кнопки для зміни простору.

- `on_AddingQuest_clicked()`: Обробник події натискання кнопки для додавання нового квесту.

- `on_QuestList_itemDoubleClicked(QListWidgetItem *item)`: Обробник події подвійного натискання на елемент у списку квестів.

- `on_more_characteristics_clicked()`: Обробник події натискання кнопки для перегляду більше характеристик персонажа.

- `on_Open_inventory_clicked()`: Обробник події натискання кнопки для відкриття вікна інвентаря.

#### Приватні слоти

- `~MainWindow()`: Деструктор класу, звільняє пам'ять, що була виділена для об'єкту інтерфейсу головного вікна.
...
// Додавання активного квесту до персонажа
Quest newQuest;
mainWindow.addActiveQuest(&newQuest);

// Оновлення інформації про персонажа та квести у вікні
mainWindow.updateInfoOnCharacter();
mainWindow.updateInfoOnQuest();

// Обробка подвійного натискання на елемент у списку квестів
mainWindow.on_QuestList_itemDoubleClicked(item);

// Оновлення інформації про характеристики персонажа
mainWindow.updateInfoOnCharacter();

// Виконання завершення квесту та оновлення характеристик персонажа
mainWindow.questComplete();
```
